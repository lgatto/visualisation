...
file](https://github.com/lgatto/visualisation/raw/master/data/hiiragi2013.rda).
...
Below, we combine the (transposed) expression values (extraced from
the `hiiragi2013` object with `exprs`) and sample metadata (extraced
...
And, adding coulours representing the different samples
...
Let's start by exploring some 1 dimensional visualisation. First, we
convert a microarray gene expression data to a data.frame, fit for
some `ggplot2` visualisation, focusing on genes Fgf4, Gata4, Gata6 and
Sox2.
...
**Exercise:** Repeat the above figure replaceing the boxes by violins
...
It is also possibly to visualise all points to get a sense of their
distribution.
...
- **Histogram** requires definition of bins (width, positions) and can
  create visual artifacts especially if the number of data points is
  not large.
...
```{r sc1, fig.cap = "Scatter plot comparing the expression of two probesets."}
dfx <- as.data.frame(Biobase::exprs(hiiragi2013))
scp <- ggplot(dfx, aes(x= `59 E4.5 (PE)`, y = `92 E4.5 (FGF4-KO)`))
scp + geom_point()
```

**Exercise:** The overplotting of the dots stops us from learning
...
When visualising data along additional dimenstion, we can parameterise
the points by setting their shape, colour, size and transparency, that
can be set with point aesthetics such as `fill`, `color`, `shape`,
`size` and `alpha`.
...
```{r facet1, fig.cap = "Different sub-plots for different lineages."}
ggplot(dftx, aes(x = X1426642_at, y = X1418765_at)) +
  geom_point() +
    facet_grid( . ~ lineage )
```
...
**Exercise:** Use facets to visualise the distribution of four Fgf4,
Gata4, Gata6 and Sox2 genes in the `genes` data.
